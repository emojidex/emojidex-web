...
var AutoComplete = function () {
  ...
  _createClass(AutoComplete, [{
    ...
    value: function setAutoComplete() {
      ...
      var setAtwho = function setAtwho(at_options) {
        $(_this2.plugin.element).atwho(at_options).on('reposition.atwho', function (e) {
      ...
      var setSearchedEmojiData = function setSearchedEmojiData(at_obj, match_string) {
        var updateAtwho = function updateAtwho(search_results, at_bak) {
          var at_options = {
            data: search_results,
            callbacks: {
              highlighter: onHighlighter,
              matcher: function matcher(flag, subtext, should_startWithSpace) {
        ...
        var num = ++_this2.searching_num;
        _this2.EC.Search.search(match_string, function (response) {

          var search_results = [];
          for (var i = 0; i < _this2.EC.Search.results.length; i++) {
            var emoji = _this2.EC.Search.results[i];
            search_results.push({
              code: emoji.code.replace(/\s/g, '_'),
              img_url: '' + _this2.EC.cdn_url + _this2.EC.size_code + '/' + emoji.code.replace(/\s/g, '_') + '.png',
              insert_tag: _this2.EC.Util.emojiToHTML(emoji)
            });
          }

          if (_this2.searching_num === num) {
            if (search_results.length) {
              updateAtwho(search_results, at_obj);
            }
            _this2.searching_num = 0;
          }
        });
      ...
      var getRegexp = function getRegexp(flag, should_startWithSpace) {
        return new RegExp('[\uFF1A' + flag + ']([^\uFF1A:;@&#~!$+?%*\f\n\r\\/]+)$', 'gi');
      };

      var getMatchString = function getMatchString(subtext, regexp) {
        var match = regexp.exec(subtext);
        match = match ? match[2] || match[1] : null;
        return match;
      };

      var onHighlighter = function onHighlighter(li, query) {
        if (!query) {
          return li;
        }
        var regexp = new RegExp('>\\s*([^:;@&#~!$+?%*\f\n\r\\/]*?)(' + query.replace(/(\(|\))/g, '\\$1') + ')([^:;@&#~!$+?%*\f\n\r\\/]*)\\s*<', 'ig');
      ...
      var at_init = {
        ...
        callbacks: {
          highlighter: onHighlighter,
          matcher: function matcher(flag, subtext, should_startWithSpace) {
            var match = getMatchString(subtext, getRegexp(flag, should_startWithSpace));
            if (match) {
...
var Palette = function () {
  ...
  _createClass(Palette, [{
    ...
    key: 'setCodeList',
    value: function setCodeList(kind, code_list) {
      var _this4 = this;
      ...
      for (var i = 0; i < code_list.length; i++) {
        ...
        emoji_button.append(emoji_button_image);
        emoji_button.click(function (e) {
          return _this4.insertEmojiAtCaret($(e.currentTarget).prop('emoji_data'));
        });
    ...
    value: function insertEmojiAtCaret(emoji) {
      ...
      if (elem.is('[contenteditable="true"]')) {
        ...
        var txt = elem.val();
        var startTxt = txt.substring(0, pos);
        var stopTxt = txt.substring(pos, txt.length);
    ...
    value: function toggleSorting() {
      ...
      if (this.EC.User.auth_info.premium) {
        return function () {
          var result = [];
          var iterable = _this5.getInitializedTabs();
          for (var i = 0; i < iterable.length; i++) {
            var tab = iterable[i];
            var item = void 0;
            if (!tab.tab_content.find('#sort-selector').length) {
              item = tab.tab_content.find('ul.pagination').after(_this5.getSorting(tab));
            }
            result.push(item);
          }
          return result;
        ...
        return this.getInitializedTabs().map(function (tab) {
          return _this5.removeSorting(tab);
        });
      }
    }
  }, {
...
//# sourceMappingURL=category_tab.js.map
  ...
  _createClass(UserTab, [{
    ...
    value: function setHistory(auth_info) {
      ...
      return this.palette.EC.User.History.get(function (response) {
        return _this4.setDataByCodes(response.map(function (item) {
          return item.emoji_code;
        }), _this4.palette.EC.User.History.meta, 'history');
      });
    }
  }, {
    key: 'setFavorite',
    value: function setFavorite(auth_info) {
      ...
      return this.palette.EC.User.Favorites.get(function (response) {
        return _this5.setData(response, _this5.palette.EC.User.Favorites.meta, 'favorite');
      });
    }
  }, {
    key: 'setData',
    ...
    value: function setDataByCodes(data, meta, kind) {
      ...
      var tab_pane = $('<div class=\'tab-pane ' + (active ? 'active' : '') + '\' id=\'tab-content-user-' + kind + '\'></div>');
    ...
    key: 'getPagination',
    value: function getPagination(meta, pane, kind) {
      var _this6 = this;
      ...
      if (kind === 'favorite') {
        var callback = function callback(response) {
          ...
          return _this6.setData(response, _this6.palette.EC.User.Favorites.meta, kind);
        };
        prev_func = function prev_func() {
          return _this6.palette.EC.User.Favorites.prev(callback);
        };
        next_func = function next_func() {
          return _this6.palette.EC.User.Favorites.next(callback);
        };
      } else {
        var _callback = function _callback(response) {
          $('#tab-content-user-history').remove();
          return _this6.setDataByCodes(response.map(function (item) {
            return item.emoji_code;
          }), _this6.palette.EC.User.History.meta, kind, true);
        };
        prev_func = function prev_func() {
          return _this6.palette.EC.User.History.prev(_callback);
        };
        next_func = function next_func() {
          return _this6.palette.EC.User.History.next(_callback);
        };
      }
    ...
    _createClass(Plugin, [{
      ...
      value: function replace() {
        ...
        if (this.options.autoUpdate) {
          ...
          this.replacer = new Replacer(this).loadEmoji().then(function () {
